// Package cmd 中的 skill.go 实现 skill 子命令，为 AI Coding 工具生成 SKILL.md 指令文件
package cmd
...
var skillInitCmd = &cobra.Command{
	Use:   "init",
	Short: "为 AI Coding 工具生成 gongfeng CLI 的 SKILL.md",
	Long:  "扫描当前目录，交互式选择 AI Coding 工具，为选中的工具生成 gongfeng CLI 的 SKILL.md 指令文件。",
	RunE:  runSkillInit,
}
...
func runSkillInit(cmd *cobra.Command, args []string) error {
	...
	fmt.Fprintf(os.Stderr, "已为 %d 个工具生成 gongfeng skill：\n", len(generated))
...
func printMenu(w *os.File, selected []bool) {
	fmt.Fprintln(w, "请选择要初始化 gongfeng skill 的 AI 编程工具：")
	for i, tool := range supportedTools {
		mark := " "
		if selected[i] {
			mark = "x"
...
// writeSkillFile 将 SKILL.md 写入指定工具的 skills/gongfeng/ 目录
func writeSkillFile(cwd, toolDir string, content []byte) (string, error) {
	dir := filepath.Join(cwd, toolDir, "skills", "gongfeng")
